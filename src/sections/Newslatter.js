...
const Newslatter = () => {
  return (
    <div className='bg-[url("/public/assets/images/newslatter-sm.svg")] sm:bg-[url("/public/assets/images/newslatter.svg")] bg-cover relative mt-14'>
      ...
      />
      <div className="absolute inset-0 max-w-[1030px] px-14 py-8 flex items-center mx-auto">
        <div className="flex flex-col sm:flex-row  justify-between w-full">
          <div className="flex flex-col h-full justify-between">
            <h1 className="font-orbitron text-white font-bold md:text-3xl sm:text-2xl lg:text-4xl text-xl">
          ...
          <img src="/assets/images/line.svg" className="max-h-[94px] hidden sm:flex self-start lg:px-10 sm:px-4"/>
          <div>
            <p className="font-pop text-white text-[17px] my-7 sm:my-0">
              SIGN UP BELOW TO RECEIVE ALL THE LATEST UPDATES FROM Debetz HQ
            </p>
          </div>
          <div className="rounded-lg border-opacity-60 border-white border-[0.01px] bg-[#0E0E1C]  flex sm:hidden justify-between sm:mt-5 lg:mt-12 max-w-sm ">
                <input  className="text-sm py-3 pl-4 text-white font-pop bg-transparent outline-none border-none" placeholder="Enter your mail"/>
                <button className="h-full py-3 px-5 bg-[#BC4AF2] rounded-lg text-white font-pop text-sm font-semibold scale-105">Subscribe</button>    
            </div>
        </div>
